...
class TestTransform(unittest.TestCase):
    def test_hb_transform(self):
        ...
        expected = OrderedDict([('gender', 'female'), ('accountid', '1'), ('country', 'Germany'), 
                                ('age', 42), ('extract_date', '2019-03-07'), ('load_date', '2019-03-07')])
        result = Transform.hb_transform(record, datetime.strptime('2019-03-07', 
    ...
    def test_wwc_transform(self):
        ...
        expected = {'gender': 'female', 'accountid': 'U9LROYgO', 'country': 'France', 'age': 73, 
                    'extract_date': '2019-03-07', 'load_date': '2019-03-07'}
        result = Transform.wwc_transform(record, datetime.strptime('2019-03-07', 
                                         transform.EXTRACT_DATE_FORMAT).date())
        self.assertEqual(result, expected)

    def test_transform(self):
        def trans_fun(record, extract_date):
        ...
        expected = [{'baz': 2}, {'baz': 3}]
        result = Transform.transform(data, trans_fun, datetime.strptime('2019-03-07', 
                                    transform.EXTRACT_DATE_FORMAT).date())
        result = list(result)
        self.assertEqual(result, expected)
